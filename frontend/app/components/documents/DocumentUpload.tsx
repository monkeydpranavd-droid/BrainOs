...
export function DocumentUpload({
  ...
  const processUpload = async (item: UploadFileItem) => {
    ...
    try {
      // Execute document upload Service call
      ...
      // Invalidate react-query cache for documents list
      ...
      setFilesList((prev) =>
        prev.map((f) => (f.id === item.id ? { ...f, status: "failed", error: error.detail || "Upload failed" } : f))
      );
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div
        ...
        className={`border border-dashed rounded-2xl p-8 text-center flex flex-col items-center justify-center cursor-pointer transition-all duration-205 ${
          isDragOver
      ...
      >
        <input
          ...
          multiple
          className="hidden"
          accept=".pdf,.docx,.doc,.txt,.csv,.md,.markdown,.pptx,.ppt,image/*"
        />
        <div className="h-12 w-12 bg-slate-950/80 border border-slate-850 rounded-xl flex items-center justify-center mb-4 text-slate-400 group-hover:text-slate-200">
          <UploadCloud className="h-6 w-6" />
        </div>
        <p className="text-sm font-semibold text-slate-250">
          Drag and drop your document here, or <span className="text-indigo-400 font-bold hover:underline">browse files</span>
        </p>
        <p className="text-xs text-slate-500 mt-2.5 leading-relaxed">
          Supports PDF, DOCX, TXT, CSV, MD, PPTX and Images (up to 50MB)
        </p>
      </div>
            ...
            <div key={item.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0 gap-4">
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <button
                  ...
                  className="p-1.5 hover:bg-slate-950 border border-slate-850 hover:border-slate-800 text-slate-400 hover:text-red-400 rounded-lg transition-colors cursor-pointer"
